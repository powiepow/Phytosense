...
(async () => {
    try {
        ...
        console.warn("WebGL backend failed. Switching to CPU backend.");
        await tf.setBackend('cpu');
        await tf.ready();
    }
})();
...
self.onmessage = async (event) => {
    ...
    if (action === 'startTraining') {
        ...
        try {
            ...
            const { xs, ys } = await prepareDataset(dataset, numClasses);
                ...
                await model.fit(xs, ys, {
                    ...
                    callbacks: {
                        onEpochEnd: (epochIndex, logs) => {
                            self.postMessage({
                                message: `Epoch ${epoch + 1}/${epochs} completed. Loss: ${logs.loss.toFixed(4)}, Accuracy: ${logs.acc.toFixed(4)}`,
...
//function to prepare dataset tensors
async function prepareDataset(dataset, numClasses) {
    const tensors = [];
    const labels = [];

    for (const [className, images] of Object.entries(dataset)) {
        const classIndex = parseInt(Object.keys(dataset).indexOf(className));
        for (const image of images) {
            const tensor = await loadImageAsTensor(image);
            tensors.push(tensor.expandDims(0)); // Add batch dimension
            labels.push(classIndex);
        }
    }
